...
export async function runFinderAgent(): Promise<{ leadsFound: number; runtimeLimitHit: boolean }> {
  ...
  // daily_lead_limit = global daily send limit (master outbound cap).
  // Used here as the total lead discovery budget for the run.
  ...
  logger.info('finder', 'Targets', { globalDailySendLimit: TOTAL_TARGET, initialOutreachLimit: EMAIL_TARGET, dmTarget: DM_TARGET })
  logger.info('finder', '[OUTBOUND_ALLOCATION]', {
  ...
  logger.info('finder', `GLOBAL_DAILY_SEND_LIMIT = ${TOTAL_TARGET}`)
  logger.info('finder', `INITIAL_OUTREACH_LIMIT = ${EMAIL_TARGET}`)
          ...
          for (const result of results) {
            ...
            if (foundEmail) {
              ...
              if (emailCount >= EMAIL_TARGET) {
                logger.info('finder', 'EMAIL_TARGET_REACHED', {
                  email_count:         emailCount,
                  email_target:        EMAIL_TARGET,
