...
function dependencies(
  ...
  return {
    client,
    telemetry: observability(workspaceId) as unknown as OrchestratorDependencies['telemetry'],
    ...
    acquireLeadLock: (leadId) => acquireLock(client, `orchestrator:lead:${leadId}`, LEAD_LOCK_TTL_MS, workspaceId),
    releaseLeadLock: (leadId, token) => releaseLock(client, `orchestrator:lead:${leadId}`, token, workspaceId),
  }
}
...
export async function runLeadOrchestration(request: OrchestrationRequest): Promise<OrchestrationResult> {
  ...
  const client = createWorkspaceServiceClient(request.workspaceId) as SupabaseClient<Database>
  return orchestrateLead(request, dependencies(client, request.workspaceId))
}
...
export async function runConfiguredLeadBatch(input: {
  ...
  const client = createWorkspaceServiceClient(input.workspaceId) as SupabaseClient<Database>
  ...
  const requests = input.leadIds.map((leadId): OrchestrationRequest => ({
    workspaceId: input.workspaceId, workflowType: 'lead_lifecycle', leadId, source: input.source,
    triggerRunId: input.triggerRunId, parentRunId: input.parentRunId,
    correlationId: input.correlationId, attempt: input.attempt,
    shadow: false,
  }))
  return orchestrateLeadBatch(requests, (request) => orchestrateLead(request, dependencies(client, input.workspaceId, contextByLeadId.get(request.leadId))))
}
