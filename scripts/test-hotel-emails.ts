...
function filterEmail(email: string): boolean {
  ...
  // Short alphanumeric with digits = generated ID — real locals don't mix random chars+digits
...
async function run() {
  console.log('Searching Outscraper: "hotel Sydney" limit=10...\n')
  const results = await searchBusinesses('hotel Sydney', 10)
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (!r.website) {
      console.log('    ❌ No website — no email anywhere')
      console.log('    → No email — would check Instagram')
      noEmail++
      continue
    }
    ...
    // a+b. Fetch homepage — check mailto: first, then full HTML regex
    console.log(`    Fetching homepage...`)
    const { html: homeHtml, status: homeStatus } = await fetchHtml(r.website)
    console.log(`    HTTP ${homeStatus} (${homeHtml.length} chars raw HTML)`)

    if (homeHtml) {
      const { mailto, regex } = findEmails(homeHtml)
      console.log(`    mailto: links found: ${mailto.length ? mailto.join(', ') : '(none)'}`)
      console.log(`    regex matches found: ${regex.length ? regex.slice(0, 5).join(', ') : '(none)'}`)

      if (mailto.length) {
        foundEmail = mailto[0]
        source = 'mailto on homepage'
      } else if (regex.length) {
        foundEmail = regex[0]
        source = 'homepage regex'
      }
    }
  ...
  console.log('\n' + '═'.repeat(60))
  console.log(`Out of ${results.length} hotels:`)
