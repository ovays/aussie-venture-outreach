...
assert.match(filteredCsv, /^Business,Email Addresses,Mailbox,Provider,Last Subject,ReachAgent Status,Received,Sent,Email Status,Last Activity,Last Direction\r\n/)
...
assert.match(componentSource, /Unable to load email activity from the selected mailbox/, 'provider-neutral mailbox error state is rendered')
assert.match(componentSource, /\/api\/mailboxes/, 'readable mailbox connections are loaded for provider selection')
